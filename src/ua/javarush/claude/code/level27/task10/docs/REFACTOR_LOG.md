...
- Phase: Phase 1 — pilot slice.
- Owner: команда Billing Platform.
- Checks: `./gradlew test --tests "*Characterization*"`, `git diff --stat` залишається малим.
- Next boundary: увімкнути новий шлях лише за feature flag `cashflow.mrr.v2-single-plan`.

## Відкриті питання

- Success criteria для всієї дорожньої карти поки не зафіксовані централізовано.
- Rollback описано лише на рівні окремого slice, загального phase-rollback немає.