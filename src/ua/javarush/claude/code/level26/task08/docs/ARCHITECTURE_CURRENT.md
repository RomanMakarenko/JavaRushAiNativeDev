...
- Клас `com.cashflow.payments.PaymentGateway` проводить списання та перевіряє webhook.
- Ключ провайдера і webhook secret читаються з properties через env-змінні.
- Тестом покрито лише happy path `charge()`; `verifyWebhook()` і refund не покриті.

## Зв’язаність

- `payments` викликається з MRR, із refund flow і з білінгу — висока coupling.